'''
PhD 2 --- create_salinity clusters
'''
...
print('__ Importing salinity budgets')
salinity_budget = {}
for expt in experiments: 
    salinity_budget[expt] = xr.open_dataset(
        f"/nfs/see-fs-01_users/eeymr/database/{expt}/perso/salinity_budgets/{expt}.salinity_budget.decadal.nc").salinity_budget    
...
for expt in experiments:
    ...
    for zone in mask_cluster.keys():
        ...
        for depth in ['sfc', 'int', 'deep','tot']:
            cluster = (salinity_budget[expt].sel(
                depth_1=slice(*depths[depth]), drop=True)*mask_cluster[zone]).sum(
    ...
    salinity_clusters[expt] = (xr.concat(list(ds_zone.values()), dim='zone')).rename('salinity_cluster')


    # Saving the salinity clusters
    print(f"__ Saving salinity clusters for {expt}")

    sav_folder = f"/nfs/see-fs-01_users/eeymr/database/{expt}/perso/salinity_budgets"
    util.makedir(sav_folder)
    
    util.nc_sav(salinity_clusters[expt], f"{sav_folder}/{expt}.salinity_clusters.decadal")
